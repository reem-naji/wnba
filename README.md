...
As a WNBA fan, I noticed there wasn't a rigorous, data-driven way to track the MVP race in real time — most "predictions" during a season are narrative-driven takes rather than models grounded in historical voting patterns. This project builds that: a model trained on every MVP vote since 1997, applied to live 2026 season data, with the reasoning behind each prediction made transparent through feature importance analysis.
...
6. **Explainability** — since native feature importances aren't comparable across XGBoost/LightGBM/CatBoost (different internal metrics), unified importance is computed by combining **SHAP values** from all three models using the same weights as the prediction ensemble (`explain.py`)
...
- `explain.py` — computes ensemble-level SHAP feature importance for the dashboard's explainability chart
...
wnba-predictor/
├── airflow/
│   └── dags/                    # scaffolded for future pipeline orchestration (not yet deployed)
├── ingestion/
│   ├── extractors/
...
**Dashboard:** Streamlit
**Planned:** Docker, Apache Airflow, FastAPI
...
- [ ] Automate weekly data refresh (scrape → predict → update dashboard) via a scheduled job
- [ ] Containerize the API and pipeline with Docker for reproducible deployment
- [ ] Implement the scaffolded Airflow DAGs for production orchestration
